...
trait EventSourcingCommandHandlerTrait
{
    ...
    protected function doHandle($command, callable $onHandled = null): void
    {
        ...
        if (!$handler instanceof DomainEventHandlerInterface) {
            throw new \LogicException('Object "%s" is unable to handle domain event "%s". Did you forgot to implement DomainEventHandlerInterface?', get_class($handler), get_class($event));
        }
